...
func NewUser(user string, password string) *ZeppLifeUser {
	...
	u := &ZeppLifeUser{IsPhone: isPhone, User: user, Password: password}
	return u

}
func (u *ZeppLifeUser) CheatOnSteps(stepCnt int) error {
	logger := log.WithField("user", u.User)
	//1.获取access
	...
	logger.WithField("accessCode", accessCode).Info("print accessCode")
	...
	logger.WithField("loginResp", loginResp).Info("print loginResp")
	time.Sleep(5 * time.Second)
	//3.调用接口刷步数
...
func (u *ZeppLifeUser) login(code string) (*LoginResp, error) {
	...
	if err != nil {
		log.WithError(err).Error("login post err")
		return nil, err
	}
	bs, err := io.ReadAll(resp.Body)
	if err != nil {
		log.WithError(err).Error("resp read all err")
		return nil, err
	}
	loginResp := &LoginResp{}
	err = json.Unmarshal(bs, loginResp)
	if err != nil {
		log.WithError(err).Error("resp unmarshal err")
		return nil, err
	}
...
func (u *ZeppLifeUser) getAccess() (string, error) {
	...
	if resp.StatusCode != 303 {
		log.WithField("status code", resp.StatusCode).Error("resp code error")
...
func (u *ZeppLifeUser) pushStep(appToken string, stepCnt int) error {
	...
	log.WithField("api", api).Info("push step")
	hc := http.Client{}
	...
	req.Header.Set("User-Agent", "Mozilla/5.0 (iPhone; CPU iPhone OS 14_7_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.1.2")
	resp, err := hc.Do(req)
	if err != nil {
		return err
	}
	...
	log.WithField("pushStepResp", pushStepResp).Info("print pushStepResp")
	if pushStepResp.Code != 1 || pushStepResp.Message != "success" {
		log.Error("push step error, pls check resp error")
...
func (u *ZeppLifeUser) buildStepData(stepCnt int) map[string]string {
	...
	log.WithField("datajson", dataJson).Info("print data json")
	return map[string]string{
		"userid":              u.Uid,
